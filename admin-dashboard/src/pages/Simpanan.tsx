...
import { Plus, Trash2, Wallet, FileText, Check, X, Eye } from 'lucide-react';
...
const Simpanan: React.FC = () => {
  ...
  const [simpananTab, setSimpananTab] = useState('Semua');
  ...
  const filteredData = simpananTab === 'Semua' 
    ? dataList 
    : dataList.filter(item => item.jenisSimpanan === simpananTab);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => { setFormData(initialForm); setBuktiFile(null); setIsDrawerOpen(true); }} className="btn btn-primary flex items-center gap-2 shadow-lg">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100/50 text-sm">
                {filteredData.map((item, index) => (
                  <tr key={item.id} className="hover:bg-white/40 transition-colors align-top group">
                    ...
                    <td className="px-4 py-3.5 font-bold text-emerald-600">Rp {item.saldo.toLocaleString('id-ID')}</td>
                    <td className="px-4 py-3.5">
                      <div className="flex flex-col items-start gap-1">
                        {getStatusBadge(item.status)}
                        {item.buktiUrl && (
                          <a href={`${BASE_URL}${item.buktiUrl}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline text-[10px] flex items-center gap-1 mt-1">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center justify-center gap-1">
                        {item.status === 'MENUNGGU_PERSETUJUAN' && (
                          <>
                            <button onClick={() => handleUpdateStatus(item.id, 'DISETUJUI')} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Setujui"><Check size={16} /></button>
                            <button onClick={() => handleUpdateStatus(item.id, 'DITOLAK')} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Tolak"><X size={16} /></button>
                          </>
                        )}
                        <button onClick={() => handleDeleteClick(item.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
